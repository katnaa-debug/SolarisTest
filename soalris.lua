...
local function AddSoftOrbs(parent, theme)
	...
	local function Spawn(size, color)
		local Orb = Create("Frame", {
			Parent = Container, Size = UDim2.fromOffset(size, size),
			Position = UDim2.fromScale(math.random(5,95)/100, math.random(5,95)/100),
			AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = color,
			BackgroundTransparency = 0.92, BorderSizePixel = 0, ZIndex = 1
		})
		AddCorner(Orb, 1000)
		Create("UIGradient", {Parent = Orb, Transparency = NumberSequence.new{NumberSequenceKeypoint.new(0, 0.5), NumberSequenceKeypoint.new(1, 1)}})
		task.spawn(function()
			while Orb.Parent do
	...
	for i = 1, 10 do Spawn(math.random(100, 350), (i % 2 == 0) and theme.Accent or theme.Second) end
end
...
function Library:CreateWindow(Settings)
	...
	function Library:Notify(Config)
		...
		local Icon = Create("ImageLabel", {
			Parent = Frame, Size = UDim2.new(0, 32, 0, 32), Position = UDim2.new(0, 12, 0.5, -16),
			BackgroundTransparency = 1, Image = Image, ImageColor3 = SelectedTheme.Accent, ZIndex = 10002
		})
		
		-- Texts
		Create("TextLabel", {
			Parent = Frame, Size = UDim2.new(1, -60, 0, 25), Position = UDim2.new(0, 55, 0, 10),
			BackgroundTransparency = 1, Text = Title, Font = Enum.Font.GothamBold, TextColor3 = SelectedTheme.Text, TextSize = 15, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 10002
		})
		Create("TextLabel", {
			Parent = Frame, Size = UDim2.new(1, -60, 0, 20), Position = UDim2.new(0, 55, 0, 35),
			BackgroundTransparency = 1, Text = Content, Font = Enum.Font.Gotham, TextColor3 = SelectedTheme.TextDark, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 10002, TextWrapped = true
		})
		...
		-- Timer Animation (Synchronized)
		local TimerTween = TweenService:Create(Bar, TweenInfo.new(Duration, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 1, 0)})
		TimerTween:Play()
		
		-- Wait for timer to finish, THEN fade out
		task.spawn(function()
			TimerTween.Completed:Wait() -- Perfectly synced wait
			TweenService:Create(Frame, TweenInfo.new(0.4, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = UDim2.new(1, 50, 0, 0), BackgroundTransparency = 1}):Play()
			task.wait(0.4)
			Frame:Destroy()
		end)
	end
	...
	local TabContainer = Create("ScrollingFrame", { Parent = Main, Size = UDim2.new(0, 150, 1, -105), Position = UDim2.new(0, 10, 0, 45), BackgroundColor3 = SelectedTheme.Second, BackgroundTransparency = 0.5, ScrollBarThickness = 0, BorderSizePixel = 0, ZIndex = 5 }); AddCorner(TabContainer, 8)
